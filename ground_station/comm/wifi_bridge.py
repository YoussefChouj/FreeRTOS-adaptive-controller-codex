...
class WifiBridge:
    ...
    def start(
        ...
        # Bind to the MicoAir interface IP with an ephemeral port.
        # Binding to 0.0.0.0:14550 does NOT receive MicoAir frames on Windows
        # (the MicoAir sends to the specific interface IP, not the wildcard).
        # Binding to 192.168.4.2:14550 and sending from the same socket also does
        # NOT work — the MicoAir apparently suppresses data when the nudge source
        # port equals its own RX port (14550).  The solution: bind to
        # 192.168.4.2:0 (ephemeral port on the specific IP) and use the SAME
        # socket for both sending the nudge and receiving the data frames.
        # The MicoAir responds to the nudge source port (ephemeral) and the OS
        # routes the response to the socket bound to that same ephemeral port.
        self._wifi.bind(("192.168.4.2", 0))
        self._wifi.settimeout(0.1)

        # Nudge: 1-byte datagram to tell the MicoAir to aim its downlink at us.
        # Sent on the same socket as RX so the MicoAir's response goes to our RX.
        try:
            self._wifi.sendto(b"\x00", (self._nudge_host, self._nudge_port))
        except OSError:
            pass

        # Separate send socket for command frames (0xCC 0xDD) that must originate
        # from port 14550 (the MicoAir's command port).
        self._wifi_send = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self._wifi_send.bind(("192.168.4.2", 14550))
    ...
    def _send_cmd_frame(self, cmd_id: int, index: int, value: float) -> None:
        ...
        try:
            self._wifi_send.sendto(frame, (self._wifi_host, self._wifi_port))
        except OSError as e:
